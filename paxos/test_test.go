...
func TestForget(t *testing.T) {
  ...
  fmt.Println("checking if min advanced")
...
func TestManyForget(t *testing.T) {
  ...
  go func() {
    for done == false {
      ...
      if seq >= pxa[i].Min() {
        ...
        if decided {
          //fmt.Println("telling", i, "done", seq)
          pxa[i].Done(seq)
        }
      }
      runtime.Gosched()
  ...
  fmt.Println("checkpoint here for after sleep")
  done = true
  for i := 0; i < npaxos; i++ {
...
func TestPartition(t *testing.T) {
  ...
  for iters := 0; iters < 20; iters++ {
    ...
    part(t, tag, npaxos, []int{0,1,2}, []int{3,4}, []int{})  //xxxxxxxxxxxxxxcxxxxxxxxxxxxxxxxxxx
